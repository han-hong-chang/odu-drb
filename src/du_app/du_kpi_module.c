...
void kpmStoreCellRlcMetric(CellPmList* cellMetricList){
    if(cellMetricList != NULL){
        if(cellMetricList->numUe>0){
            ...
            kpmCellPmDb.eachRlcPm[indexOfCellRlcPm].ThpDl = cellMetricList->cellRecord.ThpDl;
        }
    }
...
/*******************************************************************
 *
 * @brief Store the Drb Metrics by RLC
 *
 * @details
 *
 *    Function : kpmStoreDrbRlcMetric
 *
 *    Functionality: Store the Drb Metrics by RLC
 *
 * @param[in] drbMetricList, DrbPmList* drb metrics
 * @return void
 *
 ******************************************************************/

void kpmStoreDrbRlcMetric(DrbPmList* drbMetricList){
    if(drbMetricList != NULL){
        if(drbMetricList->numUe>0){
            printf("\nINFO   -->  E2SM-KPM : Support sending UE Average Throughput to xApp");
            if(indexOfDrbRlcPm<SIZE_OF_RLCDB){
                indexOfDrbRlcPm++;
            }
            else{
                indexOfDrbRlcPm=0;
            }
            kpmDrbPmDb.eachRlcPm[indexOfDrbRlcPm].ThpDl = drbMetricList->drbRecord.ThpDl;
        }
    }
    if(kpmDrbIndicationEnable){
        indicationRlcDrbCount++;
    }
}
/*modified by joe*/
/*******************************************************************
