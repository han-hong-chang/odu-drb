...
/*******************************************************************
*
* @brief Handles received Drb Metrics from RLC and forward it to E2 
*
* @details
*
*    Function : DuProcRlcDrbMetrics
*
*    Functionality: 
*      Handles received Drb Metrics from RLC and forward it to E2
*
* @params[in] Post structure pointer
*              DrbPmList *drbStats
*
...
*
* ****************************************************************/
uint8_t DuProcRlcDrbMetrics(Pst *pst, DrbPmList *drbStats)
{
    uint8_t drbRecord = 0;

    DU_LOG("\nDEBUG  -->  DU APP : Received Drb Metrics");
    ...
    for(drbRecord = 0; drbRecord < drbStats->numUe; drbRecord++)
    {
       DU_LOG("\nINFO   -->  DU_APP: UEID:%d,  UlTput:%.5lf",drbRecord+1,\
                        drbStats->drbRecord[drbRecord].ThpDl);

    }      /*從這邊觀察rlc丟過來的資料是否有成功傳到這邊*/
   ...
   kpmStoreDrbRlcMetric(drbStats);


   DU_FREE_SHRABL_BUF(pst->region, pst->pool,drbStats->ueRecord, (drbStats->numUe) * (sizeof(UePm)));
   DU_FREE_SHRABL_BUF(pst->region, pst->pool,drbStats, sizeof(DrbPmList));
