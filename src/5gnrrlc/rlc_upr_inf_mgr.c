...
uint8_t rlcCalculateTputPerDrb(CmLListCp *drbList, Direction dir)
{
   ...
   /*modify by joe*/
   DrbPmList drbPm;
   DrbPm record[8];   /*UE是7所以應該是7但我怕有問題先多寫一個*/
   /*modify by joe*/
...
/*modify by joe*/
   if(dir == DIR_DL){
      drbPm.drbRecord =  (DrbPm*)calloc(drbCnt, sizeof(DrbPm));
      drbPm.numUe = drbCnt;
      for(int i=0;i<drbCnt;i++){
         memcpy(&drbPm.drbRecord[i], &record[i], sizeof(DrbPm));      /* rlcThptCb->drbTputInfo.dlTputPerDrbList[ueIdx]             MAX_NUM_UE=7*/
      }
      BuildDrbReportToDu(&drbPm, drbCnt);
      free(drbPm.drbRecord);
   }
/*modify by joe*/
   return(drbCnt);
}
