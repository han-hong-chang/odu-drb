...
/*******************************************************************
*
* @brief Send the Drb Metrics to  DU APP
*
* @details
*
*    Function : sendDrbPmToDu
*
*    Functionality:
*      Handles the sending of Drb Metrics to  DU APP
*
* @params[in] Post structure pointer
*             DrbPmList *drbStats pointer
*
* @return ROK     - success
*         RFAILED - failure
*
* ****************************************************************/
uint8_t sendDrbPmToDu(DrbPmList *drbStats)
{
   Pst pst;  
   
   FILL_PST_RLC_TO_DUAPP(pst, RLC_UL_INST, EVENT_RLC_DRB_PM_TO_DU);

   if(!drbStats)
   {
      DU_LOG("\nERROR  -->  RLC: sendDrbPmToDu(): Memory allocation failed ");
      return RFAILED;
   }
   else
   {
      if(rlcSendDrbPmToDu(&pst, drbStats) == ROK)
      {
         DU_LOG("\nDEBUG  -->  RLC: Drb PM send successfully");
      }
      else
      {
         DU_LOG("\nERROR  -->  RLC: sendDrbPmToDu():Failed to send Drb PM to DU");
         RLC_FREE_SHRABL_BUF(pst.region, pst.pool, drbStats, sizeof(DrbPmList));
         return RFAILED;
      }
   }
   return ROK;
...
/*******************************************************************
*
* @brief Builds the Drb Performance Metrics structure to be sent to DU
*
* @details
*
*    Function : BuildDrbReportToDu
*
*    Functionality:
*      Builds the Drb Performance Metrics structure to be sent to DU
*
* @params[in] uint8_t uelCnt
*             
* @return ROK     - success
...
uint8_t BuildDrbReportToDu(DrbPmList *drbPm, uint8_t drbCnt)
{
   CmLList  *node = NULLP;
   RlcTptPerDrb *drbNode = NULLP;
   DrbPmList *drbStats = NULLP;   /*Drb metric */

   if(drbCnt == 0)
   {
      DU_LOG("\nERROR  -->  RLC: No DRB to send the Drb PM");
      return RFAILED;
   }

   RLC_ALLOC_SHRABL_BUF(RLC_MEM_REGION_UL, RLC_POOL, drbStats, sizeof(DrbPmList));
   if(drbStats == NULLP)
   {
      DU_LOG("\nERROR  -->  RLC: Memory Allocation Failed");
      return RFAILED;
   ...
   RLC_ALLOC_SHRABL_BUF(RLC_MEM_REGION_UL, RLC_POOL, drbStats->drbRecord, drbCnt * (sizeof(DrbPm)));

   if(drbStats->drbRecord == NULLP)
   {
      DU_LOG("\nERROR  -->  RLC: Memory Allocation Failed");
      RLC_FREE_SHRABL_BUF(RLC_MEM_REGION_UL, RLC_POOL, drbStats, sizeof(DrbPmList));
      return RFAILED;
   }
   for(int i=0;i<drbCnt;i++){
      memcpy(&drbStats->drbRecord[i], &drbPm->drbRecord[i], sizeof(DrbPm));
   }
   drbStats->numUe = drbPm->numUe;
   sendDrbPmToDu(drbStats);
