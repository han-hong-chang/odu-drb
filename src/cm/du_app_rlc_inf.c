...
/*******************************************************************
*
* @brief Packs and Sends Drb PM from RLC to DUAPP
*
* @details
*
*    Function : packRlcDuDrbPm
*
*    Functionality:
*       Packs and Sends Drb Performance Metrics from RLC to DUAPP
*
*
* @params[in] Post structure pointer
*             DrbPmList *drbStats
...
uint8_t packRlcDuDrbPm(Pst *pst, DrbPmList *drbStats)
{
   Buffer *mBuf = NULLP;
   ...
   if(pst->selector == ODU_SELECTOR_LWLC)
   {
      if (ODU_GET_MSG_BUF(pst->region, pst->pool, &mBuf) != ROK)
      {
         DU_LOG("\nERROR  --> RLC : Memory allocation failed at packRlcDuDrbPm");
         return RFAILED;
      }
      /* pack the address of the structure */
      CMCHKPK(oduPackPointer,(PTR)drbStats, mBuf);
   }
   else
   {
      DU_LOG("\nERROR  -->  RLC: Only LWLC supported for packRlcDuDrbPm");
      return RFAILED;
   }
...
/*******************************************************************
*
* @brief Unpacks Drb PM received from RLC
*
* @details
*
*    Function : unpackRlcDrbPm
*
*    Functionality:
*         Unpacks Drb Performance Metrics received from RLC
*
* @params[in] Pointer to Handler
*             Post structure pointer
*             Message Buffer
...
uint8_t unpackRlcDrbPm(RlcDrbPmToDuFunc func, Pst *pst, Buffer *mBuf)
{
    if(pst->selector == ODU_SELECTOR_LWLC)
    {
       DrbPmList *drbStats = NULLP;
       /* unpack the address of the structure */
       CMCHKUNPK(oduUnpackPointer, (PTR *)&drbStats, mBuf);
       ODU_PUT_MSG_BUF(mBuf);
       return (*func)(pst, drbStats);
    }
    else
    {
       /* Nothing to do for other selectors */
       DU_LOG("\nERROR  -->  RLC: Only LWLC supported for Drb Metrics ");
       ODU_PUT_MSG_BUF(mBuf);
    }
