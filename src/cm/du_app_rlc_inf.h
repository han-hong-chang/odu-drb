...
/*modified by joe*/
#define EVENT_RLC_DRB_PM_TO_DU 224
/*modified by joe*/
#define RB_ID_SRB 0
#define RB_ID_DRB 1
...
/*modified by joe*/
/*Drb Metric for NW Slicing from RLC to DUAPP */
typedef struct drbPm
{ 
  double ThpDl;  /*only DL so different with SlicePm*/
}DrbPm;

typedef struct drbPmList
{  
   DrbPm *drbRecord;
   uint8_t numUe;
   UePm *ueRecord;
...
/*modifid by joe*/
/* Drb Metrics from RLC to DU APP */
typedef uint8_t (*RlcDrbPmToDuFunc) ARGS((
   Pst           *pst,
   DrbPmList *drbStats));
/*modifid by joe*/
